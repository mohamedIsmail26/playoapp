...
const CreateActivityScreen = () => {
  ...
  const [taggedVenue, setTaggedVenue] = useState(null); // Store venue as an object
  const { user } = useUser();
  ...
  const createGame = async () => {
    try {
      const response = await axios.post('http://localhost:3001/api/games/create', {
        sport,
        area: taggedVenue?.name, // Use venue name for API
        date,
        time: timeInterval,
        admin: user?.id,
      ...
      console.error('Failed to create game:', err.response?.data || err.message);
      ...
      <ScrollView className="px-4">
        ...
        >
          ...
          <View className="flex-1">
            ...
            <TextInput
              value={taggedVenue?.name || ''} // Display venue name
        ...
        >
          ...
          <View className="flex-1">
            ...
            <TextInput
              editable={false}
              value={date} // Use value for display
        ...
        >
          ...
          <View className="flex-1">
            ...
            <TextInput
              value={timeInterval} // Use value for display
              editable={false} // Prevent manual editing
