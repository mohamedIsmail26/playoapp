    ...
    <SafeAreaView className="flex-1 bg-gray-50">
      ...
      <View className="bg-[#294461] p-4 pb-6">
        ...
        <View className="mt-4 flex-row justify-between items-center">
          <Text className="text-white text-lg font-semibold">Players ({players.length})</Text>
          <View className="flex-row gap-2 items-center">
            <Globe color="white" size={24} />
            <Text className="text-white text-base">{activityAccess === 'public' ? 'Public' : 'Invite Only'}</Text>
          </View>
        </View>
      </View>
          ...
          {players.length === 0 ? (
            ...
            players.map((player, index) => (
              <TouchableOpacity
                ...
                activeOpacity={0.8}
                onPress={() => {/* Future: Navigate to player profile */}}
              >
                <Image
                  source={{ uri: player.imageUrl || 'https://i.pravatar.cc/100' }}
