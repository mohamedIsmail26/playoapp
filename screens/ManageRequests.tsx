...
const ManageRequests: React.FC = () => {
  const [option, setOption] = useState<'Requests' | 'Invited' | 'Playing' | 'Retired'>('Requests');
  ...
  const fetchRequests = async () => {
    try {
      const response = await axios.get(`http://localhost:3001/api/games/${gameId}/requests`);
      console.log('Requests response:', JSON.stringify(response.data, null, 2));
      setRequests(response.data);
    } catch (error: any) {
      console.error('Failed to fetch requests:', error.message, error.response?.data);
  ...
  const fetchPlayers = async () => {
    try {
      const response = await axios.get(`http://localhost:3001/api/games/game/${gameId}/players`);
      console.log('Players response:', JSON.stringify(response.data, null, 2));
      setPlayers(response.data);
    } catch (error: any) {
      console.error('Failed to fetch players:', error.message, error.response?.data);
  ...
  const toggleMatchFull = async () => {
    try {
      const response = await axios.post('http://localhost:3001/api/games/toggle-match-full', {
        gameId,
        adminId: user?.id,
      });
      ...
      console.error('Failed to toggle match full:', error.message, error.response?.data);
  ...
  const acceptRequest = async (userId: string) => {
    try {
      const response = await axios.post('http://localhost:3001/api/games/accept', {
        gameId,
        userId,
        adminId: user?.id,
      });
      ...
      console.error('Failed to accept request:', error.message, error.response?.data);
        ...
        <View className="flex-row justify-between mt-4">
          {['Requests', 'Invited', 'Playing', 'Retired'].map((tab) => (
            ...
            >
              ...
              >
                {tab} ({tab === 'Requests' ? requests.length : tab === 'Playing' ? players.length : 0})
              </Text>
            </TouchableOpacity>
          ))}
        </View>
        ...
        {loading ? (
          ...
          <>
                  ...
                  requests.map((item, index) => (
                    ...
                    >
                      <View className="flex-row items-center">
                        <Image
                          source={{ uri: item.imageUrl || 'https://i.pravatar.cc/100' }}
            ...
            {option === 'Playing' && (
              <View>
                {players.length === 0 ? (
                  <Text className="text-gray-500 text-center mt-10">No players yet</Text>
                ) : (
            ...
            {option === 'Invited' && (
              <Text className="text-gray-500 text-center mt-10">No invited players</Text>
            )}
            {option === 'Retired' && (
              <Text className="text-gray-500 text-center mt-10">No retired players</Text>
            )}
            <View className="h-8" />
          </>
        )}
