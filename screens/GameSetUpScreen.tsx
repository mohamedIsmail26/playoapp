...
interface Game {
  ...
  isUserAdmin: boolean;
  requests: Request[];
  queries?: any[]; // Added to handle game.queries in Queries section
}

const GameDetailsScreen: React.FC = () => {
  const navigation = useNavigation<any>(); // Use any for navigation type if not typed
  ...
  // Modal animation for swipe-down
    ...
    const fetchData = async () => {
      try {
        ...
        const playersResponse = await axios.get(
          `http://localhost:3001/api/games/game/${game._id}/players`
        );
        console.log(
          "Fetched players:",
        ...
        const requestsResponse = await axios.get(
          `http://localhost:3001/api/games/${game._id}/requests`
        );
        console.log(
          "Fetched requests:",
  ...
  const sendJoinRequest = async () => {
    try {
      const response = await axios.post(
        `http://localhost:3001/api/games/${game._id}/request`,
        {
          userId,
          comment,
        }
  ...
  const sendQuery = async () => {
    try {
      const response = await axios.post(
        `http://localhost:3001/api/games/${game._id}/queries`,
        {
          userId,
          query,
        }
  ...
  const toggleMatchFullStatus = async () => {
    try {
      const response = await axios.post(
        "http://localhost:3001/api/games/toggle-match-full",
        {
          gameId: game._id,
        }
      );
