...
const SlotScreen: React.FC = () => {
  ...
  // Generate time slots (5:00 AM to 11:00 PM, 30-minute intervals)
  ...
  const AVAILABILITY = TIME_SLOTS.map((time, index) => {
    ...
    return isBooked ? 2 : 1; // 2 = booked, 1 = available
  });

  // Generate dates (today + 4 days)
  ...
  // Create a new game if gameId is not provided
  useEffect(() => {
    const createNewGame = async () => {
      ...
      try {
        const response = await axios.post(
          "http://localhost:3001/api/games/create",
          {
            sport: selectedSport,
            area: place,
            date: selectedDate,
  ...
  const handleCourtSelection = (courtNumber: string, courtName: string) => {
    // Check if the court is booked for the selected time
    const timeIndex = TIME_SLOTS.indexOf(selectedTime);
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `http://localhost:3001/api/games/${currentGameId}/book`,
        {
          userId,
  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-100">
      {/* Sticky Header with Checkout Button */}
