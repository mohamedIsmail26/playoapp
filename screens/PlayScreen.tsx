...
const PlayScreen: React.FC = () => {
  ...
  const [selectedCategory, setSelectedCategory] = useState<'Calendar' | 'Recommended' | 'My Sports' | 'Other Sports' | 'Past Games'>(
  ...
  const fetchGames = useCallback(async () => {
    console.log('Fetching games - selectedCategory:', selectedCategory, 'userId:', user?.id);
    ...
    if (selectedCategory === 'Calendar') {
      endpoint = 'http://localhost:3001/api/games/upcoming';
      ...
      endpoint = 'http://localhost:3001/api/games/games';
    }
    ...
    try {
      ...
      console.error(`Failed to fetch games for ${selectedCategory}:`, error.message, error.response?.data);
      setGames([]);
  ...
  const checkCompletedGames = useCallback(async (gameList: Game[]) => {
    const now = moment();
    const completedGames = gameList.filter((game) => {
      const [startTime, endTime] = game.time.split(' - ');
      const gameEndTime = moment(`${moment(game.date, 'Do MMMM', true).format('YYYY-MM-DD')} ${endTime}`, 'YYYY-MM-DD h:mm A');
      return gameEndTime.isValid() && gameEndTime.isBefore(now);
    });

    if (completedGames.length > 0) {
      const playerClerkIds = completedGames.flatMap(game => game.players.map(player => player._id) || [user?.id]);
      try {
        await axios.post('http://localhost:3001/api/users/update-game-stats', { playerClerkIds });
        if (user?.id) {
  ...
  const displayGames = useMemo(() => {
    ...
    if (selectedCategory === 'Past Games') {
      return filteredGames.filter(game => {
        ...
        const gameDateTime = moment(`${gameDate.format('YYYY-MM-DD')} ${gameTime}`, 'YYYY-MM-DD h:mm A');
      ...
      return filteredGames
        .filter(game => {
          const [startTime, endTime] = game.time.split(' - ');
          const gameStartTime = moment(`${moment(game.date, 'Do MMMM', true).format('YYYY-MM-DD')} ${startTime}`, 'YYYY-MM-DD h:mm A');
          const gameEndTime = moment(`${moment(game.date, 'Do MMMM', true).format('YYYY-MM-DD')} ${endTime}`, 'YYYY-MM-DD h:mm A');
          return gameEndTime.isAfter(now) || (gameStartTime.isBefore(now) && gameEndTime.isAfter(now));
        })
        .sort((a, b) => {
          const aStart = moment(`${moment(a.date, 'Do MMMM', true).format('YYYY-MM-DD')} ${a.time.split(' - ')[0]}`, 'YYYY-MM-DD h:mm A');
          const bStart = moment(`${moment(b.date, 'Do MMMM', true).format('YYYY-MM-DD')} ${b.time.split(' - ')[0]}`, 'YYYY-MM-DD h:mm A');
          return aStart.isBefore(bStart) ? -1 : 1;
        });
    } else if (selectedCategory === 'My Sports') {
      return filteredGames.sort((a, b) => (b.createdAt ? new Date(b.createdAt) : 0) - (a.createdAt ? new Date(a.createdAt) : 0));
    }
    return filteredGames;
  ...
  const handleCancelRequest = () => {
    setNeedsRefresh(true); // Trigger re-fetch after cancellation
  };
      ...
      <View className="bg-[#1f2937] pb-3">
        <View className="px-4 pt-3 flex-row justify-between items-center">
          <View className="flex-1">
            ...
            <Text className="text-white text-lg font-semibold">Sahakar Nagar, Bengaluru</Text>
          </View>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} className="mt-4 px-4">
          {['Calendar', 'Recommended', 'My Sports', 'Other Sports', 'Past Games'].map((item, index) => (
            <TouchableOpacity
              key={index}
              className="mr-6"
              onPress={() => {
            ...
            >
              <Text
                className={`text-base font-bold ${
                  selectedCategory === item ? 'text-green-400' : item === 'Recommended' ? 'text-gray-500' : 'text-white'
                }`}
              >
                {item}
              </Text>
          ...
          {[
            ...
            <TouchableOpacity
              ...
              className={`mr-3 px-4 py-2 rounded-full flex-row items-center gap-2 ${
                selectedSport === sport.name ? 'bg-green-500' : 'bg-white border border-gray-200'
              }`}
