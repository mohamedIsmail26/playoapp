...
// CREATE OR UPDATE USER (Called after debut)
...
router.post('/update-game-stats', async (req, res) => {
  const { playerClerkIds } = req.body; // Array of clerkIds for all players
  ...
  try {
    const updatePromises = playerClerkIds.map(async (clerkId) => {
      ...
      if (user) {
        // Increment gameCount
        user.gameCount += 1;
        // Add all other players as playpals (excluding self)
        const newPlaypals = playerClerkIds.filter((id) => id !== clerkId);
        user.playpals = [...new Set([...user.playpals, ...newPlaypals])]; // Avoid duplicates
        await user.save();
      }
    });
