...
router.post("/:gameId/book", async (req, res) => {
  try {
    ...
    // Check for conflicting bookings with the same sport
    ...
    // Ensure all existing bookings have sport (temporary fix until data migration)
    venue.bookings = venue.bookings.map((booking) => ({
      ...booking,
      sport: booking.sport || sport, // Fallback to requested sport if missing
    }));
